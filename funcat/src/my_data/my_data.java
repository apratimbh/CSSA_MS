...
{
	String file="latest"; //data fine name
	double[][] arr = null;
	...
	File file1 = new File("./data/cellcycle.owl");  // ontology file name
	...
	try {
		vertex=(ArrayList<String>) new ObjectInputStream(new FileInputStream("vertex1")).readObject(); // 1 for funcat and 2 for go
		for(String s : vertex)
		{
		...
		int c=0,i;
		String curr;
		pbr = new BufferedReader(new FileReader("./data/"+file+".txt"));
		...
		br = new BufferedReader(new FileReader("./data/"+file+".txt"));
		flag=false;
		c=0;
	...
	try {
		for(int j=0;j<a2;j++)
		{
			bw=new BufferedWriter(new FileWriter("./data/new/data-"+j+".txt"));
			bw.write("%\n");
